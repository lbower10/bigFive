 ...
 * Author: Prof. Arnold, with code added for PP10-11 by
 * Logan Bowers and Matthew Krahel
 * Decription: Contains a templated BinarySeachTree class with functions,
 * constructors, etc. used to operate on the tree
 
 * **preorderTraverseAndWrite added for Lab 4 
 */
...
public:
   ...
   void preorderTraverseAndWrite( ostream& outBin, const Position<T>& );
...
/*
 * preorderTraverseAndWrite: Performs a preorder write to
 * 	a file
 * Parameters: An outstream& and a const position<T>&
 * Returns: Nothing
 */
template<class T>
void BinarySearchTree<T>::preorderTraverseAndWrite( ostream& outBin, const Position<T>& p) 
{
	if(p.isInternal())
	{
		T item = p.getItem();
		item.write(outBin);
		preorderTraverseAndWrite( outBin, p.nodePtr->leftPtr );
		preorderTraverseAndWrite( outBin, p.nodePtr->rightPtr );
	}
}
...
// Copy constructor: Performs a deep cody of one tree
// to another tree
template<class T>
 ...
 */
 template<class T>
 Node<T>* BinarySearchTree<T>::copyTree(Node<T>* p, const Node<T>* n)
 {
	 Node<T>* newNode;
	if( n != NULL) 
	{
		newNode = Node<T>(*n); // so, new node has n’s data
		newNode->parentPtr = p;
		newNode->leftPtr = copyTree( newNode, n->leftPtr );
		newNode->rigthPtr = copyTree( newNode, n->rightPtr );
		return newNode;
	}
  ...
  * Parameters: Const reference to a Tree
  * Returns: A Tree& with parameter data copied into it
  */
template<class T>
BinarySearchTree<T>& BinarySearchTree<T>::operator=(const BinarySearchTree<T>& b)
{
	...
	if(b.empty())
	{
		...
		this->superRootPtr->rightPtr->parentPtr = b.superRootPtr;
	}
	else 
	{
		this->superRootPtr->rightPtr = copyTree( this->superRootPtr, b.superRootPtr->rightPtr );
	}
